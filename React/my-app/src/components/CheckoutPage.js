...
function CheckoutPage() {
  ...
  const handleConfirmPurchase = async () => {
    ...
    try {
      const response = await fetch('http://localhost:3000/placeOrder', { // Ensure the endpoint matches your backend
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        if (result.success) {
          ...
          alert('Failed to confirm purchase: ' + result.message);
        }
      } else {
        const result = await response.json();
        alert('Failed to confirm purchase: ' + result.message);
      ...
      alert('An error occurred while confirming purchase');
    }
  };
