...
const updateProduct = async (req, res) => {
  try {
    const client = await MongoClient.connect('mongodb://localhost:27017');
    const coll = client.db('grocery').collection('products');
    const productName = req.body.ProductName; // Using ProductName to identify the product
    const productData = req.body;
...
// Listing orers for admin panel

const listOrders = async (req, res) => {
  try {
    const client = await MongoClient.connect('mongodb://localhost:27017/', { useUnifiedTopology: true });
    const retailerOrdersColl = client.db('grocery').collection('retailerOrders');
    const orders = await retailerOrdersColl.find().toArray();
    client.close();
...
module.exports = { home, signUp, signIn, adminSignUp, adminSignIn, addProduct, updateProduct, deleteProduct, getUser,addToCart,removeFromCart,updateCart,getCart, updateUser, getProducts ,placeOrder, getOrders,listOrders};
